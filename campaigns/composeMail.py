...
if sys.argv[1] == "1":
    ...
    if sys.argv[2] == "all_role6":
        ...
        a = str("select distinct(email),CONCAT(first_name,' ',last_name) as full_name,username from users_customuser where role=6 AND email NOT IN (SELECT email FROM home_unsubscriber)")

    if sys.argv[2] == "all_role4":
        today = date.today()
        a = str("select distinct(email),CONCAT(first_name,' ',last_name) as full_name,username from users_customuser where role=4 AND email NOT IN (SELECT email FROM home_unsubscriber)")

    if sys.argv[2] == "job_applicants":
        a = str("select distinct(email),CONCAT(first_name,' ',last_name) as full_name from home_job where email NOT IN (SELECT * FROM home_unsubscriber)")
    ...
    if sys.argv[2] == "nigerian_internship":
        a = str("select email, CONCAT(firstname,' ',lastname) as full_name, course from home_internship where email NOT IN (SELECT email FROM home_unsubscriber) ")

    if sys.argv[2] == "switch_career":
        a = str("select email, fullname as full_name, new_career_id, application_date from home_careerswitchapplication where email NOT IN (SELECT email FROM home_unsubscriber) ")

    if sys.argv[2] == "yesterdays_switch_career":
        a = str("select email, fullname as full_name, new_career_id, application_date from home_careerswitchapplication where email NOT IN (SELECT email FROM home_unsubscriber) AND application_date >= DATE_SUB(NOW(), INTERVAL 25 HOUR) ")

    if sys.argv[2] == "part_time_fresh_grads":
        a = str("select email, fullname, phone from home_parttimejob where email NOT IN (SELECT email FROM home_unsubscriber) ")
    ...
    if sys.argv[2] == "yesterdays_interested_job_applicants":
        a = str("select email, fullname as full_name, phone, position_id, interest, application_date from home_job where email NOT IN (SELECT email FROM home_unsubscriber) and interest='interested' AND application_date >= DATE_SUB(NOW(), INTERVAL 25 HOUR) ")
    ...
    if sys.argv[2] == "test":
        ...
        if sys.argv[3] == "yesterdays":
            ...
            a = str("SELECT DISTINCT(u.email), CONCAT(u.firstname,' ',u.lastname) as full_name,u.username,u.id,u.role,u.date_created  FROM users_customuser as u where username='%s'" % ("joseph",))
...
if sys.argv[1] == "2":
    if sys.argv[2] == "b":
        a = str("select id,first_name,last_name,username from users_customuser")
    elif sys.argv[2] == "c":
...
#print(sys.argv[3])
slug=sys.argv[3]
b= str(""" select title, message, slug from home_message WHERE slug = '%s'""" % (slug,))
...
if sys.argv[2] != "insert":
    ...
    for row in results:
        ...
        if sys.argv[2] == "test":
            ...
            os.system('python emailer.py %s %s %s %s %s' % (TEST_EMAIL, TESTER, Title, message, extra_msg))
        else:
            os.system('python emailer.py %s %s %s %s %s' % (row[0], full_name, Title, message, extra_msg))
