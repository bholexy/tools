...
for student in fundamentalsStudents:
    ...
    MSG = """export LJUMAIL=%s && export LJUNAME=%s  && for i in {1..9}; do echo '=============  LAB '$i'  ============='; %s/daily.py 1 k all | grep %s | grep LinuxFundamentalsLab_$i | ack --passthru 'failed'; done | mutt -s 'Current Fundamentals Lab Report for %s ' -- showpopulous@gmail.com folay1881@yahoo.com %s"""  % ( studentEmail, studentUsername, REPORTS_HOME, studentUsername, studentUsername, studentEmail)
